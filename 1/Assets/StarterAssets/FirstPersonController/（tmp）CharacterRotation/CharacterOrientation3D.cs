...
public class CharacterOrientation3D : CharacterAbility
{
    ...
    protected override void Initialization()
    {
        ...
        _characterHandleWeapon = _character.gameObject.GetComponentInChildren<PlayerWeaponsManagerBeta>();
    ...
    protected virtual void RotateToFaceWeaponDirection()
    {
        ...
        {
            if (_rotationDirection.magnitude >= AbsoluteThresholdWeapon)
            {
                if (Input.GetMouseButtonDown(0))
                {
                    Debug.Log("the distance is " + _rotationDirection.magnitude);
                }
